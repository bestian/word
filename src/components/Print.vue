<template>
  <div class="hello">
    <h1 class="o" v-for="i in getRange(max)" :key="i">
      <img id = "main" alt="字" :src="'https://www.moedict.tw/' + (msg || ' ') + '.png'" :style="{'width': (msg.split('').length == 1 ? ft*3 : ft*6) + 'vw', margin: '-' + (msg.split('').length == 1 ? 2 / msg.split('').length : ft * 1.5 + 2 / msg.split('').length) + 'vw 0'}">
    </h1>
    <form class="no-print">
